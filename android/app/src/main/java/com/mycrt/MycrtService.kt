...
class MycrtService : Service() {
    private val db by lazy { MycrtApp.database }
    private val scope = CoroutineScope(Dispatchers.IO)
    private val CHANNEL_ID = "mycrt_service_channel"

    override fun onCreate() {
        super.onCreate()
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, "Mycrt System", NotificationManager.IMPORTANCE_LOW)
            val manager = getSystemService(NotificationManager::class.java)
            manager.createNotificationChannel(channel)
        }
        val notification = NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Mycrt Automation")
